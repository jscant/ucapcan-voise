...
%Myf = fittype('a/x + b');
%f = fit(nsa', tVDa_cppb'./nda', Myf)
%p = plot(f, nsa, tVDa_cppb./nda, 'x'); 
%set([p(1) p(2)], 'color', 'k', 'markersize', 4);
...
%hold on;
%plot(nsr(end), (tVDr_cppb(end)+15e-4)/nsr(end), 'ko', 'markersize', 12);
%hold on;
%plot(nsa(1), tVDa_cppb(1)/nda(1), 'ko', 'markersize', 12);
